...
def refine_detections(rois, probs, deltas, window, config):
    ...
    if config.DETECTION_MIN_CONFIDENCE:
        keep = np.intersect1d(
            keep, np.where(class_scores >= config.DETECTION_MIN_CONFIDENCE)[0]) ## Find the intersection of two arrays.
    ...
    nms_keep = []
    for class_id in np.unique(pre_nms_class_ids): ## Find the unique elements of an array
    ...
    """
        e.g.,
        np.argsort([5, 3, 2, 7, 8, 1]) = 
            array([5, 2, 1, 0, 3, 4])

        np.argsort([5, 3, 2, 7, 8, 1])[::-1] = 
            array([4, 3, 0, 1, 2, 5])
    """
    keep = keep[top_ids]
...
class DetectionLayer(KE.Layer):
    ...
    def call(self, inputs):
        ...
        return tf.py_func(wrapper, inputs, tf.float32) ## Wraps a python function and uses it as a TensorFlow op
