...
class BatchNorm(KL.BatchNormalization):
    ...
    def call(self, inputs, training=None):
        return super(self.__class__, self).call(inputs, training=False) ## super() 函数用于调用下一个父类(超类)并返回该父类实例的方法。
...
def fpn_classifier_graph(rois, feature_maps,
    ...
    pool_size: The width of the square feature map generated from ROI Pooling. POOL_SIZE = 7
    ...
    x = KL.TimeDistributed(BatchNorm(axis=3), name='mrcnn_class_bn1')(x) ## This wrapper allows to apply a layer to every temporal slice of an input.
    ...
    shared = KL.Lambda(lambda x: K.squeeze(K.squeeze(x, 3), 2),
                       name="pool_squeeze")(x) ## remove size 1 dimensions by specifying axis.

    # Classifier head
    # [batch, boxes, num_classes]
    mrcnn_class_logits = KL.TimeDistributed(KL.Dense(num_classes),
                                            name='mrcnn_class_logits')(shared) ## tf.keras.layers.TimeDistributed: This wrapper allows to apply a layer to every temporal slice of an input.
...
def build_fpn_mask_graph(rois, feature_maps,
    ...
    feature_maps: List of feature maps from diffent layers of the pyramid,
                  [P2, P3, P4, P5]. Each has a different resolution.
    image_shape: [height, width, depth]
    pool_size: The width of the square feature map generated from ROI Pooling. MASK_POOL_SIZE = 14
    num_classes: number of classes, which determines the depth of the results
...
class MaskRCNN():
    ...
    def compile(self, learning_rate, momentum):
        ...
        self.keras_model.add_loss(tf.add_n(reg_losses)) ## tf.add_n: Adds all input tensors element-wise.
...
def trim_zeros_graph(boxes, name=None):
    ...
    non_zeros: [N] a 1D booleaNn mask identifying the rows to keep
    """
    non_zeros = tf.cast(tf.reduce_sum(tf.abs(boxes), axis=1), tf.bool)
    boxes = tf.boolean_mask(boxes, non_zeros, name=name) ## Apply boolean mask to tensor. Numpy equivalent is tensor[mask]
