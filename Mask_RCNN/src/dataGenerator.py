...
def build_rpn_targets(image_shape, anchors, gt_class_ids, gt_boxes, config):
    ...
    if crowd_ix.shape[0] > 0:
        ...
        no_crowd_bool = (crowd_iou_max < 0.001) ## no_crowd_bool[i] = true, means non crowd anchor
    else:
    ...
    gt_iou_argmax = np.argmax(overlaps, axis=0) ## don't keep any GT box unmatched (rare, but happens). Instead, match it to the closest anchor (even if its max IoU is < 0.3).
    ...
    while True:
        try:
            ...
            if b >= batch_size:
                ...
                yield inputs, outputs ## 带有 yield 的函数在 Python 中被称之为 generator（生成器）; 执行到 yield 时，函数就返回一个迭代值，下次迭代时，代码从 yield 9的下一条语句继续执行
