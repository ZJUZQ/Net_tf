...
class PyramidROIAlign(KE.Layer):
    ...
    def call(self, inputs):
        ...
        roi_level = log2_graph(tf.sqrt(h * w) * tf.sqrt(image_area) / 224.0 )
        ...
        for i, level in enumerate(range(2, 6)):
            ix = tf.where(tf.equal(roi_level, level)) ## indexs of roi_level where value is level 
            ...
            level_boxes = tf.stop_gradient(level_boxes) ## Stops gradient computation
